...
- 2026-02-21 | PR: pending | Phase: 3 Data Consistency | Replaced `ExerciseSwapScreen` mock alternative exercise dataset with live backend exercise search results (muscle-filtered, searchable, and return-path aware), so swap flow uses real API data instead of hardcoded entries.
