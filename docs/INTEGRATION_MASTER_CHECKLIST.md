...
- 2026-02-21 | PR: `TBD` | Phase: 3 Stability Fix | Further fixed floating resume-workout opening empty state by making backend `/workouts/current` prefer non-empty in-progress workouts when duplicates exist and adding one-time `ActiveWorkout` forced resync + loading gate before rendering empty-state UI.
