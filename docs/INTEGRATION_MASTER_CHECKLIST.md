...
- 2026-02-21 | PR: `TBD` | Phase: 3 Stability Fix | Fixed floating resume-workout pill opening empty workouts by hardening `syncCurrentWorkout` stale-state recovery (clear stale local in-progress session when backend has none, force sync when local exercise map is empty) and making pill tap sync current workout before navigating to `ActiveWorkout`.
