...
- 2026-02-21 | PR: pending | Phase: 3 Data Consistency | Fixed backend routine query boolean parsing so `isTemplate=false` and `isArchived=false` are interpreted correctly (instead of truthy string coercion), restoring correct routine/workout list retrieval for frontend screens that pass explicit boolean filters.
