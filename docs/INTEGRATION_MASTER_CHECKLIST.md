...
- 2026-02-21 | PR: `1a6f4fe` | Phase: Global Navigation Hardening | Root-caused bottom-nav overlap to absolute tab bar being visible across nested stack screens; fixed centrally in `MainTabs` by showing tab bar only on root tab screens (`HomeDashboard`, `WorkoutHub`, `ExploreHub`, `SocialHome`, `ProfileHub`) and hiding it on pushed detail/flow screens (timer, AI generator, etc.).
- 2026-02-21 | PR: `e26ceac` | Phase: 3/5 Workout Tracking Reliability | Added rest timer pause/resume/extend support in workout store + active session UI, fixed rest overlay control overlap by reserving bottom space and adding explicit secondary controls, normalized workout API exercise payloads (`primaryMuscleGroups` -> `muscleGroup` compatibility), and fixed muscle-volume derivation on workout tracking views so completed-set muscle visuals render from real backend data.
- 2026-02-21 | PR: `49d3f2b` | Phase: 3/5 Active Session UX Hardening | Fixed floating resume-workout deep navigation across nested navigators, added manual rest timer start action in active workout tracking UI, made timer minimize action pause by default with visible paused minimized state, added collapse/expand control for live body visualizer, and improved targeted-muscle list usability with clipping + show-all/show-less expansion controls.
- 2026-02-21 | PR: `7320a9f` | Phase: 3 Stability Fix | Fixed floating resume-workout pill opening empty workouts by hardening `syncCurrentWorkout` stale-state recovery (clear stale local in-progress session when backend has none, force sync when local exercise map is empty) and making pill tap sync current workout before navigating to `ActiveWorkout`.
- 2026-02-21 | PR: `bc91753` | Phase: 3 Stability Fix | Further fixed floating resume-workout opening empty state by making backend `/workouts/current` prefer non-empty in-progress workouts when duplicates exist and adding one-time `ActiveWorkout` forced resync + loading gate before rendering empty-state UI.
