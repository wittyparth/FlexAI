...
function RoutineRow({ routine, onPress, colors }: { routine: any; onPress: () => void; colors: ThemeColors }) {
    const diffColor = DIFF_COLOR[routine.difficulty] || colors.chart4;
    return (
        <TouchableOpacity style={[styles.routineRow, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={onPress} activeOpacity={0.75}>
            ...
            <View style={styles.routineInfo}>
                ...
                <Text style={[styles.routineMeta, { color: colors.mutedForeground }]}>{routine.daysPerWeek}×/wk • {routine.exercises || '—'} exercises</Text>
            </View>
    ...
    return (
        <TouchableOpacity style={[styles.routineRow, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={onPress} activeOpacity={0.75}>
            ...
            <View style={styles.routineInfo}>
                ...
                <Text style={[styles.routineMeta, { color: colors.mutedForeground }]}>{template.exercises} exercises • Last: {template.lastUsed}</Text>
            </View>
            <Ionicons name="chevron-forward" size={18} color={colors.mutedForeground} />
        </TouchableOpacity>
    );
                        ...
                        {DUMMY_RECENT_WORKOUTS.slice(0, 3).map(w => (
                            <TouchableOpacity key={w.id} style={[styles.recentRow, { backgroundColor: colors.card, borderColor: colors.border }]} activeOpacity={0.75} onPress={() => navigation.navigate('WorkoutDetail', { workoutId: parseInt(w.id) })}>
                                ...
                                <View style={styles.recentRight}>
                                    ...
                                    {w.hasPR && <View style={[styles.prBadge, { backgroundColor: `${colors.primary.main}25` }]}><Text style={[styles.prText, { color: colors.primary.main }]}>PR</Text></View>}
                                </View>
                            </TouchableOpacity>
                        ))}
                    </View>
